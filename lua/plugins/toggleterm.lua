  ...
  specs = {
    {
      ...
      opts = function(_, opts)
        ...
        local function toggle_terminal(direction, size)
          if
            terminals[direction]
            and terminals[direction].bufnr
            and vim.api.nvim_buf_is_valid(terminals[direction].bufnr)
          then
            terminals[direction]:toggle()
          else
            local term = require("toggleterm.terminal").Terminal:new {
              direction = direction,
              size = size and function()
                return math.floor(vim.o[direction == "horizontal" and "lines" or "columns"] * size)
              end or nil,
              ...
              on_open = function(t)
                ...
                vim.keymap.set(
                  { "n", "t", "i" },
                  "<M-1>",
                  function() toggle_terminal("horizontal", 0.3) end,
                  { buffer = t.bufnr, desc = "Toggle horizontal terminal" }
                )
                vim.keymap.set(
                  { "n", "t", "i" },
                  "<M-2>",
                  function() toggle_terminal("vertical", 0.4) end,
                  { buffer = t.bufnr, desc = "Toggle vertical terminal" }
                )
                vim.keymap.set(
                  { "n", "t", "i" },
                  "<M-3>",
                  function() toggle_terminal("float", nil) end,
                  { buffer = t.bufnr, desc = "Toggle float terminal" }
                )
              end,
            }
        ...
        maps.i["<M-1>"] = {
          "<Esc><Cmd>lua require('astrocore').toggle_term_cmd({direction='horizontal', size=function() return math.floor(vim.o.lines * 0.3) end})<CR>",
        ...
        maps.i["<M-2>"] = {
          "<Esc><Cmd>lua require('astrocore').toggle_term_cmd({direction='vertical', size=function() return math.floor(vim.o.columns * 0.4) end})<CR>",
        ...
        maps.i["<M-3>"] = {
          "<Esc><Cmd>lua require('astrocore').toggle_term_cmd({direction='float', float_opts={border='rounded'}})<CR>",
